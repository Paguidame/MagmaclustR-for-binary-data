...
#' MagmaClust can be learned thanks to a SEM algorithm implemented in
#' \code{train_magmaclust_cat}. By providing a dataset, the model hypotheses
...
train_magmaclust_cat <- function(data,
  ...
  seq_elbo <- c()
    ...
    ## Initialize the affectation variables for each individual

    if (is.null(ini_affectations)) {

      affectations <-  ini_mixture(data,
                                   k = nb_cluster,
                                   name_clust = ID_k,
                                   50)

    }else if(is.data.frame(ini_affectations)){
      if(!all(c("ID", ID_k) %in% names(ini_affectations))){
        ...
        affectations <- ini_affectations
      }
    }else{
      stop("The 'ini_affecations' argument must be a data frame. Please read ",
           "?simu_affectations() for further details.")
    }
    # create the latents variables used in ve_step
    #ini_latents<- list("affectations" = affectations,"mu" = mu_k,"y_star" = y_star)


    hp_k[["prop_mixture"]] <- affectations %>%
    ...
    # run the SEM algorithm in case of categorial for MagmaclustR
      ...
      if (fast_approx) {
        ...
        seq_elbo <- elbo_monitoring_VEM(
          ...
          hyperpost=NULL,
          m_k = m_k,
          latents=post[c( "affectations","y_star","mu")],
          pen_diag,
          categorial = TRUE
        )
      ...
      new_hp <- sm_step(post$y_star,
                        ...
                        list_latents= post[c("affectations","mu")],
                        kern_k,
                        kern_i,
                        m_k,
                        common_hp_k,
      ...
      y_star<- post$y_star
      affectations<-post$affectations
